...
@MustBeDocumented
@ApiResponse(
    responseCode = "404", description = "존재하지 않는 회원 (MEMBER-001)", content = [Content(
        mediaType = "application/problem+json",
        schema = Schema(ref = "#/components/schemas/ProblemDetail"),
        examples = [ExampleObject(
            name = "memberNotFound", value = """
                    {
...
@ApiResponse(
    responseCode = "409", description = "이메일 중복 (MEMBER-002)", content = [Content(
        mediaType = "application/problem+json",
        schema = Schema(ref = "#/components/schemas/ProblemDetail"),
        examples = [ExampleObject(
            name = "duplicateEmail", value = """
                    {
...
annotation class MemberErrorResponses 