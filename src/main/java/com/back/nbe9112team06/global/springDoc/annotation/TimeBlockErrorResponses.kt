...
@ApiResponse(
    responseCode = "404", description = "모임 또는 참여자 조회 실패 (MEETING-001, PARTICIPANT-002)", content = [Content(
        mediaType = "application/problem+json",
        schema = Schema(ref = "#/components/schemas/ProblemDetail"),
        examples = [ExampleObject(
            name = "meetingNotFound", value = """
                        {"errorCode":"MEETING-001","detail":"존재하지 않는 모임입니다."}
                        """
        ), ExampleObject(
            name = "participantNotFound", value = """
...
@ApiResponse(
    responseCode = "409", description = "중복 시간표 등록 (TIMEBLOCK-005)", content = [Content(
        mediaType = "application/problem+json",
        schema = Schema(ref = "#/components/schemas/ProblemDetail"),
        examples = [ExampleObject(
            name = "duplicateTimeBlock", value = """
                    {"errorCode":"TIMEBLOCK-005","detail":"시간표가 이미 등록되었습니다."}
                    """
        )]
    )]
)
annotation class TimeBlockErrorResponses 