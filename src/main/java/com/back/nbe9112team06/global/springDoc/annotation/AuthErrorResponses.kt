 ...
 * - 인가 실패 (AUTH-005)
 */
//TODO 어노테이션 파라미터에서 .trimIndent()사용불가, 표현식은 const val만 가능
@Target(
    ...
    content = [
        Content(
            ...
            examples = [ExampleObject(
                name = "tokenMissing", summary = "토큰 없음", value = """
                                {
                ...
                name = "tokenInvalid", summary = "토큰 위조/형식 오류", value = """
                                {
                ...
                name = "tokenExpired", summary = "토큰 만료", value = """
                                {
                ...
                name = "unauthorized", summary = "인증 세션 없음 (Rq.getActor 실패)", value = """
                                {
...
@ApiResponse(
    responseCode = "403", description = "인가 실패: 접근 권한 없음 (AUTH-005)", content = [Content(
        mediaType = "application/problem+json",
        schema = Schema(ref = "#/components/schemas/ProblemDetail"),
        examples = [ExampleObject(
            name = "accessDenied", value = """
                            {
