...
import com.back.nbe9112team06.global.springDoc.example.TimeTableApiExamples
...
class TimeTableController(private val timeTableService: TimeTableService) {
    ...
    @Operation(
        summary = "시간표 조회 (자동 집계 포함)", description = """
            특정 모임의 시간표를 조회합니다.
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(
        ...
        content = [Content(
            ...
            examples = [ExampleObject(
                name = "success",
                value = TimeTableApiExamples.GET_TIMETABLE_SUCCESS_JSON
            ), ExampleObject(name = "empty", value = TimeTableApiExamples.GET_TIMETABLE_EMPTY_JSON)]
        )]
    )
    ...
    @io.swagger.v3.oas.annotations.responses.ApiResponse(
        ...
        content = [Content(
            mediaType = MediaType.APPLICATION_JSON_VALUE,
            schema = Schema(implementation = ApiResponse::class),
            examples = arrayOf(ExampleObject(name = "success", value = TimeTableApiExamples.GET_RECOMMEND_SUCCESS_JSON))
        )]
    )
    fun recommend(
        @PathVariable meetingId: Int
    ): ApiResponse<List<RecommendedScheduleResponse>> {

        return ApiResponse(
            "200-1",
            "추천 일정입니다.",
            timeTableService.recommend(meetingId)
        )
    }
