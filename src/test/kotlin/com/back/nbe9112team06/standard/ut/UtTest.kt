...
internal class UtTest {
    ...
    inner class ToString {
        ...
        fun t1_createJwt_notBlank() {
            ...
            // when
            val jwt = Ut.toString(TestConstants.JWT_SECRET, 3600L, payload)
        ...
        fun t2_jwtHasThreeParts() {
            val jwt = Ut.toString(TestConstants.JWT_SECRET, 3600L, mapOf("id" to 1))
        ...
        fun t3_differentPayload_differentJwt() {
            val jwt1 = Ut.toString(TestConstants.JWT_SECRET, 3600L, mapOf("id" to 1))
            val jwt2 = Ut.toString(TestConstants.JWT_SECRET, 3600L, mapOf("id" to 2))

            Assertions.assertThat(jwt1).isNotEqualTo(jwt2)
        }
    }
    ...
    inner class PayloadOrNull {
        ...
        fun t4_validToken_returnsPayload() {
            ...
            val jwt = Ut.toString(TestConstants.JWT_SECRET, 3600L, original)

            // when
            val payload: Claims? = Ut.payloadOrNull(jwt, TestConstants.JWT_SECRET)

            // then
            Assertions.assertThat(payload).isNotNull()
            Assertions.assertThat(payload?.get("id", Int::class.javaObjectType)).isEqualTo(1)
            Assertions.assertThat(payload?.get("nickname", String::class.java)).isEqualTo("테스터")
        }
        ...
        fun t6_malformedToken_returnsNull() {
            val payload: Claims? = Ut.payloadOrNull("not.a.valid.jwt.token", TestConstants.JWT_SECRET)
            Assertions.assertThat(payload).isNull()
        }
        ...
        fun t7_emptyString_returnsNull() {
            val payload: Claims? = Ut.payloadOrNull("", TestConstants.JWT_SECRET)
            Assertions.assertThat(payload).isNull()
        }
        ...
        fun t8_expiredToken_throwsExpiredJwtException() {
            ...
            val expiredJwt = Ut.toString(TestConstants.JWT_SECRET, -1L, mapOf("id" to 1))

            // then: null이 아니라 예외를 던져야 필터에서 구분 처리 가능
            Assertions.assertThatThrownBy { Ut.payloadOrNull(expiredJwt, TestConstants.JWT_SECRET) }
    ...
    inner class IsValid {
        ...
        fun t9_validToken_true() {
            val jwt = Ut.toString(TestConstants.JWT_SECRET, 3600L, mapOf("id" to 1))
            Assertions.assertThat(Ut.isValid(jwt, TestConstants.JWT_SECRET)).isTrue()
        }
        ...
        fun t10_differentSecret_false() {
            val jwt = Ut.toString(TestConstants.JWT_SECRET, 3600L, mapOf("id" to 1))
            Assertions.assertThat(Ut.isValid(jwt, DIFFERENT_SECRET)).isFalse()
        }
        ...
        fun t11_expiredToken_false() {
            ...
            // → 이 테스트로 현재 동작 확인
            Assertions.assertThatCode{ Ut.isValid(expiredJwt, TestConstants.JWT_SECRET) }
                .isInstanceOf(ExpiredJwtException::class.java)
        }
